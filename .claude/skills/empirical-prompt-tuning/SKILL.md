...
description: "agent 向けテキスト指示（skill / slash command / task プロンプト / コード生成プロンプト）を、バイアスを排した実行者に動かしてもらい、両面（実行者の自己申告 + 指示側メトリクス）で評価して反復改善する手法。改善が頭打ちになるまで回す。Use when user says 'プロンプトを改善して', 'スキルをチューニング', 'empirical-prompt-tuning', or after creating/heavily revising a skill or prompt."
---
...
3. **実行**: 後述の **subagent 起動契約** に従ったプロンプトを subagent に渡し、シナリオを実行させる。実行者は実装や出力を生成し、最後に自己申告レポートを返す。評価シナリオが DB 書き込みや外部サイトアクセスなど副作用を伴う場合は「副作用のあるシナリオの扱い」節に従う。
...
- 典型例: 全シナリオ `tool_uses` が 1-3 なのに 1 シナリオだけ 15+ → そのシナリオ用の recipe が skill 内に無く、docs/ や src/ を横断探索している
- 対処: iter 2 で「最小完成例 inline」や「いつ docs/ や src/ を読みに行くかの指針」を SKILL.md 冒頭に追加すると `tool_uses` は大幅低下する
...
評価シナリオの実行者（dispatch した subagent）に、副作用のある操作を本番相手に直接行わせない。

- **DB**: シナリオが DB 書き込み・更新を伴う場合、本番 DB（`arima.db`）を直接使わせない。`src/test/helpers/testDb.ts` の `createTestDb` で作る一時 SQLite DB か、`arima.db` のコピーを使わせる。イテレーションのたびに本番 DB へ書き込むと、既存データの汚染や、再評価のたびに前提が変わる非決定性を招く。
- **JRA サイト**: シナリオが出馬表の取得・抽出を伴う場合、イテレーションのたびに JRA のサイトへ実アクセスさせない。既に取得済みの HTML（`fetch-jra` の出力）や、既に抽出済みの JSON（`fetch-and-extract` の出力）を事前に用意し、それを入力としてシナリオを実行させる。反復のたびに実アクセスすると、サイト側への負荷やレート制限、取得結果の非決定性で評価がぶれる。
...
- `orchestrate` — Explore / Plan / general-purpose / review-* のサブエージェントを dispatch して開発タスクを遂行するスキル。本 skill の「新規 subagent に評価シナリオを実行させる」という dispatch の作法は orchestrate の考え方に準拠する。
- `design-principles` — 実装前に参照する設計原則。skill / プロンプトの body を大幅改訂した後、本 skill で経験的に検証する前に、記述の構造（面積・階層・秩序）が壊れていないか design-principles の観点でセルフチェックしておくと Iteration 0 の乖離検出が早まる。
